...
class ContractsController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = Contracts::select()->orderby('id', 'DESC')->paginate(10);
    ...
    {
        return view('admin.contracts.signature_image', ['id' => $request->id]);
    }
    public function store_signature_image(Request $request)
    {
        $data = Contracts::find($request->id);
        ...
        Contracts::where(['id' => $request->id])->update($data_to_update);
       $data = Contracts::find($request->id);
        return view('admin.invoice.invoice', ['data' => $data]);
    }
    /**
    ...
    public function create($id)
    {
        ...
        return view('admin.contracts.create', ['car_id' => $car_id, 'customer_id' => $customer_id]);
    }
    ...
    public function store(ContractsRequest $request)
    {
        ...
        try {
            ...
            $contract_id =  Contracts::create($data);
            /*** customer update data ****/
            $customer_id = customer::select('contract_number', 'total_money', 'paid_money', 'remaining_money')->find($request->customer_id);
            ...
            $car = Car::select('contract_number')->find($request->car_id);
    ...
    public function update(ContractsRequest $request, string $id)
    {
        ...
        try {
            ...
            $customer_id = customer::select('contract_number', 'total_money', 'paid_money', 'remaining_money')->find($request->customer_id);
            $customer_id_old = customer::select('contract_number', 'total_money', 'paid_money', 'remaining_money')->find($data->customer_id);
        ...
        debentures::where(['contract_id' => $id])->update($data_Debentures);
            ...
            Contracts::where(['id' => $id])->update($data_to_update);
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Contracts::where($field3, $operator3, $value3)->paginate(PAGINATION_COUNT);
