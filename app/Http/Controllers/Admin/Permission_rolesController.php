...
class Permission_rolesController extends Controller
{
    ...
    public function store(Permission_rolesRequest $request)
    {
        try {
            ...
            if ($checkExists == null) {
                ...
                return redirect()->route('admin.permission_roles.index')->with(['success' => __('controller.success_add')]);
            } else {
    ...
    public function update($id, Permission_rolesRequest $request)
    {
        try {
            ...
            $data = Permission_rols::select()->find($id);
            if (empty($data)) {
                return redirect()->route('admin.permission_roles.index')->with(['error' => __('controller.error_not_found')]);
            }
            ...
            return redirect()->route('admin.permission_roles.index')->with(['success' => __('controller.success_update')]);
        } catch (\Exception $ex) {
            return redirect()->back()
                ->with(['error' => __('controller.error_general') . $ex->getMessage()])
                ->withInput();
    ...
    public function details($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $data = get_cols_where_row(new Permission_rols(), ["*"], ["com_code" => $com_code, "id" => $id]);
            if (empty($data)) {
                return redirect()->route('admin.permission_roles.index')->with(['error' => __('controller.error_not_found')]);
            }
            ...
            $Permission_main_menues = get_cols_where(new Permission_main_menues(), ["id", 'name'], ["active" => 1, "com_code" => $com_code]);
            $permission_roles_main_menus = get_cols_where(new Permission_roles_main_menus(), ["*"], ["com_code" => $com_code, "permission_roles_id" => $id]);
            if (!empty($permission_roles_main_menus)) {
                foreach ($permission_roles_main_menus as $info) {
                    $info->permission_main_menues_name = get_field_value(new Permission_main_menues(), "name", ["com_code" => $com_code, "id" => $info->permission_main_menues_id]);
                    $info->added_by_admin = Admin::where('id', $info->added_by)->value('name');

                    $info->permission_roles_sub_menu = get_cols_where(new Permission_roles_sub_menu(), ["*"], ["permission_roles_main_menus_id" => $info->id]);
                    if (!empty($info->permission_roles_sub_menu)) {
                        foreach ($info->permission_roles_sub_menu as $sub) {
                            $sub->permission_sub_menues_name = get_field_value(new Permission_sub_menues(), "name", ["com_code" => $com_code, "id" => $sub->permission_sub_menues_id]);
                            $sub->added_by_admin = Admin::where('id', $sub->added_by)->value('name');

                            $sub->permission_roles_sub_menues_actions = get_cols_where(new Permission_roles_sub_menues_actions(), ["*"], ["permission_roles_sub_menu_id" => $sub->id]);
                            if (!empty($sub->permission_roles_sub_menues_actions)) {
                                foreach ($sub->permission_roles_sub_menues_actions as $action) {
                                    $action->permission_sub_menues_actions_name = get_field_value(new Permission_sub_menues_actions(), "name", ["com_code" => $com_code, "id" => $action->permission_sub_menues_actions_id]);
                                }
                            }
                        }
                    }
            ...
            return view("admin.permission_roles.details", ['data' => $data, 'Permission_main_menues' => $Permission_main_menues, 'permission_roles_main_menus' => $permission_roles_main_menues]);
        } catch (\Exception $ex) {
            return redirect()->back()
                ->with(['error' => __('controller.error_general') . $ex->getMessage()]);
        }
    ...
    public function Add_permission_main_menues($id, Request $request)
    {
        try {
            ...
            foreach ($permission_main_menues_ids as $info) {
                ...
                $checkExists = get_cols_where_row(new Permission_roles_main_menus(), ["id"], $dataToInsert);
            ...
            return redirect()->route('admin.permission_roles.details', $id)->with(['success' => __('controller.success_update')]);
        } catch (\Exception $ex) {
            return redirect()->back()
                ->with(['error' => __('controller.error_general') . $ex->getMessage()])
                ->withInput();
    ...
    public function delete_permission_main_menues($id)
    {
        try {
            ...
            if (!empty($item_row)) {
                ...
                if ($flag) {
                    return redirect()->back()
                        ->with(['success' => __('controller.success_delete')]);
                } else {
                    return redirect()->back()
                        ->with(['error' => __('controller.error_general')]);
                }
            } else {
                return redirect()->back()
                    ->with(['error' => __('controller.error_not_found')]);
            }
        } catch (\Exception $ex) {
            return redirect()->back()
                ->with(['error' => __('controller.error_general') . $ex->getMessage()]);
    ...
    public function load_add_permission_roles_sub_menu(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $permission_roles_main_menus = get_cols_where_row(new Permission_roles_main_menus(), ["id", "permission_main_menues_id"], ["id" => $request->id, 'com_code' => $com_code]);
            $permission_sub_menues = "";
            if (!empty($permission_roles_main_menus)) {
                $permission_sub_menues = get_cols_where(new Permission_sub_menues(), ["id", "name"], ["com_code" => $com_code, 'permission_main_menues_id' => $permission_roles_main_menues['permission_main_menues_id']], 'id', 'ASC');
            }
            return view('admin.permission_roles.load_add_permission_roles_sub_menu', ['permission_roles_main_menus' => $permission_roles_main_menues, 'permission_sub_menues' => $permission_sub_menues]);
        }
    }
    ...
    public function load_add_permission_roles_sub_menues_actions(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $permission_roles_sub_menu = get_cols_where_row(new Permission_roles_sub_menu(), ["id", "permission_sub_menues_id"], ["id" => $request->id]);
            $permission_sub_menues_actions = "";
            if (!empty($permission_roles_sub_menu)) {
                $permission_sub_menues_actions = get_cols_where(new Permission_sub_menues_actions(), ["id", "name"], ["com_code" => $com_code, 'permission_sub_menues_id' => $permission_roles_sub_menu['permission_sub_menues_id']], 'id', 'ASC');
            }
    ...
    public function add_permission_roles_sub_menu($permission_roles_main_menus_id, Request $request)
    {
        try {
            ...
            if (empty($permission_sub_menues_id)) {
                return redirect()->back()->with(['error' => __('controller.select_menus_first')]);
            }
            foreach ($permission_sub_menues_id as $info) {
                $dataToInsert['permission_roles_main_menus_id'] = $permission_roles_main_menues_id;
                $dataToInsert['permission_sub_menues_id'] = $info;
                $dataToInsert['permission_roles_id'] = $data['permission_roles_id'];
                $checkExists = get_cols_where_row(new Permission_roles_sub_menu(), ["id"], $dataToInsert);
                if (empty($checkExists)) {
                    $dataToInsert['added_by'] = auth()->user()->id;
                    $dataToInsert['created_at'] = now();
            ...
            return redirect()->back()->with(['success' => __('controller.success_add')]);
        } catch (\Exception $ex) {
            return redirect()->back()
                ->with(['error' => __('controller.error_general') . $ex->getMessage()])
                ->withInput();
    ...
    public function add_permission_roles_sub_menues_actions($permission_roles_sub_menu_id, Request $request)
    {
        try {
            ...
            if (empty($permission_sub_menues_actions_id)) {
                return redirect()->back()->with(['error' => __('controller.select_permissions_first')]);
            }
            foreach ($permission_sub_menues_actions_id as $info) {
                ...
                $dataToInsert['permission_roles_id'] = $data['permission_roles_id'];
                $checkExists = get_cols_where_row(new Permission_roles_sub_menues_actions(), ["id"], $dataToInsert);
                if (empty($checkExists)) {
                    $dataToInsert['added_by'] = auth()->user()->id;
                    $dataToInsert['created_at'] = now();
